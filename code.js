document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!task_name || !addButton || !tasks_block || !eduBlock || !homeBlock || !workBlock) {
    ...
    if (contentBlock) {
        ...
        if (myListElement) {
            contentBlock.insertBefore(filterBar, myListElement);
        } else {
            contentBlock.appendChild(filterBar);
        }
    }

    function sortTasks(list) {
        const copy = [...list].filter(item => item !== null && item !== undefined);
        switch (currentSort) {
            case "time-desc": 
            ...
            case "completed-first": 
                return copy.sort((a, b) => {
                    const aCompleted = a?.completed === true ? 1 : 0;
                    const bCompleted = b?.completed === true ? 1 : 0;
                    return bCompleted - aCompleted; 
                });
            case "active-first": 
                return copy.sort((a, b) => {
                    const aActive = a?.completed === false ? 1 : 0;
                    const bActive = b?.completed === false ? 1 : 0;
    ...
    function displayTasks() {
        let filtered = tasks.filter(item => item !== null && item !== undefined);
